...
struct EnumTypeStorage : public mlir::TypeStorage {
  ...
  EnumTypeStorage(uint32_t active, llvm::ArrayRef<mlir::Type> elementTypes)
  ...
  static EnumTypeStorage *construct(mlir::TypeStorageAllocator &allocator,
    ...
    return new (allocator.allocate<EnumTypeStorage>())
        EnumTypeStorage(key.first, elementTypes);
  }
...
class EnumType
  ...
  static EnumType get(std::pair<uint32_t, llvm::ArrayRef<mlir::Type>> elementTypes) {
