...
std::shared_ptr<ast::Module> parser(TokenStream &ts,
  ...
  Module module = {tokens.front().getLocation(), modulePath};
  ...
  while (tokens.size() > 0) {
    ...
    std::optional<std::shared_ptr<ast::Item>> item = tryParseItem(tokens, modulePath);
...
//        std::optional<Module> moduleOpt = tryParseModule(tokens, modulePath);
//        if (moduleOpt) {
